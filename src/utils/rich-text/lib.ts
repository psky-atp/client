...
export type FacetLink = SocialPskyRichtextFacet.Link;
export type FacetMention = SocialPskyRichtextFacet.Mention;
export type FacetRoom = SocialPskyRichtextFacet.Room;
...
export class RichTextSegment {
  ...
  get room(): FacetRoom | undefined {
    const room = this.facet?.features.find(isRoom);
    if (isRoom(room)) {
      return room;
    }
    return undefined;
...
export class RichText {
  ...
  *segments(): Generator<RichTextSegment, void, void> {
   ...
   */
  async detectFacets() {
    this.facets = detect(this.unicodeText);
    if (this.facets) {
