...
export const registerCallback = (eventNsid: SocketMessageKind, callback: (data: any) => void) =>
  callbacks.get(eventNsid)?.add(callback);
export const unregisterCallback = (eventNsid: SocketMessageKind, callback: (data: any) => void) =>
  callbacks.get(eventNsid)?.delete(callback);

// FIX: hardcoding the room uri
const socket = new WebSocket(`wss://${SERVER_URL}/subscribe?wantedRooms=${GENERAL_ROOM_URI}`);
socket.addEventListener("message", (event) => {
  ...
  switch (data.$type) {
    ...
    case "social.psky.chat.message#create":
      callbacks.get("social.psky.chat.message#create")?.forEach((cb) => cb(data));
      break;

    case "social.psky.chat.message#update":
      callbacks.get("social.psky.chat.message#update")?.forEach((cb) => cb(data));
      break;

    case "social.psky.chat.message#delete":
      callbacks.get("social.psky.chat.message#delete")?.forEach((cb) => cb(data));
      break;
  }
});
