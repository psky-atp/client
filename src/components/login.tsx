...
import type { At, SocialPskyActorProfile } from "@atcute/client/lexicons";
import {
  configureOAuth,
...
const stateIsLoggedIn = (state: LoginState) => (state?.session?.sub || state.manager) && state.rpc;
...
export const loginState = createProp<LoginState>({ pendingInit: true }, function (newState: LoginState) {
  const curr = this[0]();

  // If is logged in through OAuth
  if (!Object.keys(newState).length && stateIsLoggedIn(curr) && curr.session) {
...
export const getSessionDid = () => loginState.get().session?.sub ?? loginState.get().did;
...
const Login: Component = () => {
  ...
  onMount(() =>
    tryFor(
      ...
      (timeLeft) => setNotice(`Loading... (max. ${Math.ceil(timeLeft / 1000)}s)`),
      async () => {
  ...
  const updateNickname = async (nickname: string) => {
    const state = loginState.get();
    const sessionDid = getSessionDid();
    if (!state.rpc || !sessionDid) return;
