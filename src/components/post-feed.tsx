...
const PostFeed: Component = () => {
  ...
  const postCreateCallback = (data: PostRecord) =>
    onPostCreation(feed()?.parentElement ?? undefined, untrack(posts), data, setPosts);
  const postUpdateCallback = (data: UpdateEvent) => onPostUpdate(posts(), data);
  const postDeleteCallback = (data: DeleteEvent) => onPostDelete(posts, data, setPosts);
  onMount(() => {
  ...
  return (
    <div ref={setFeed} class="flex h-fit w-80 flex-col items-center sm:w-[32rem]">
      <div>
        <button
          type="button"
          ...
          {(record, idx) => (
            <PostItem
              ...
              isSamePoster={() =>
                ...
                record[0]().indexedAt - posts()[idx() + 1][0]().indexedAt < 600000
              }
              firstUnread={() => idx() + 1 === unreadState.get().count}
              record={record[0]}
              markAsUnread={() => unreadState.set({ count: idx() + 1, ignoreOnce: true })}
            />
          )}
        </For>
      </div>
...
function onPostDelete(
  ...
  for (const [i, p] of posts().entries()) {
    ...
    if (post.did === data.did && post.rkey === data.rkey) {
      ...
      if (i + 1 <= state.count) unreadState.set({ ...state, count: state.count - 1 });
      break;
    }
  }
}
...
function onPostCreation(
  ...
  if (scrollBox.scrollTop + scrollBox.clientHeight + 400 >= scrollBox.scrollHeight) {
    toScroll = true;
  }

  setter([
    createSignal<PostRecord>(newPost as PostRecord),
    ...currPosts.slice(0, currPosts.length < MAXPOSTS ? currPosts.length : currPosts.length - 1),
  ]);
