...
const Header: Component = () => {
  ...
  onMount(() => {
    registerCallback("serverState", (data: ServerState) => setSessionCount(data.sessionCount));
  });
  ...
  return (
    <div class="mt-2 flex w-80 sm:w-[32rem]">
      <div class="flex basis-1/3 items-center gap-2 text-sm">
        ...
        >
          {theme() === "dark" ? <TbMoonStar class="size-6" /> : <TbSun class="size-6" />}
        </button>
      </div>
      ...
      <Show when={isLoggedIn()}>
        <div class="basis-1/3 text-right text-sm">
          <button type="button" class="text-red-500" onclick={() => loginState.set({})}>
            Logout
          </button>
        </div>
      </Show>
